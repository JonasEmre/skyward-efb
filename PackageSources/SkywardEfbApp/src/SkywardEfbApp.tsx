...
class SkywardEfbAppView extends AppView<RequiredProps<AppViewProps, "bus">> {
  /**
   * Optional property
   * Default view key to show if using AppViewService
   */
  protected defaultView = "AddonStatus";
  ...
  protected registerViews(): void {
    this.appViewService.registerPage("AddonStatus", () => (
      <AddonStatus appViewService={this.appViewService} />
    ));
  }
   ...
   * We usually surround it with <div class="skyward-efb-app">{super.render}</div>
   * Can render anything (JSX, Component) so it doesn't require to use AppViewService and/or AppContainer
   * @returns VNode
   */
  public render(): VNode {
    return <div class="skyward-efb-app">{super.render()}</div>;
  }
}

class SkywardEfbApp extends App {
  /**
   ...
   */
  public render(): TVNode<SkywardEfbAppView> {
    return <SkywardEfbAppView bus={this.bus} />;
  }
}
